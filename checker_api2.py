...
app = FastAPI(title="kayoosh", docs_url=None, redoc_url=None, lifespan=_lifespan)

@app.get("/kayoosh-status")
async def status():
    return JSONResponse({"ok": True, "api": "kayoosh", "dev": "@yesimtefa", **_stats})

@app.api_route("/kayoosh-xK9qPm2r", methods=["GET", "POST"])
async def check(
    request: Request,
...
if __name__ == "__main__":
    print("━" * 50)
    print("  kayoosh Checker API (NO WORKERS, UNLIMITED CONCURRENCY)")
    print("  dev     : @yesimtefa")
    print(f"  Port    : {PORT}")
    print(f"  Check   : /kayoosh-xK9qPm2r")
    print(f"  Status  : /kayoosh-status")
